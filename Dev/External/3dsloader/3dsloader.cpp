...
const int Load3DS(obj3ds_type *p_object, const char *p_filename)
{
    ...
    while( file->endOfFile() == false )
	{
		...
		//DEBUGLOG( "ChunkID: %x\n", l_chunk_id );
        file->read( &l_chunk_lenght, 4 ); //Read the lenght of the chunk
		...
		switch( l_chunk_id )
		{
			...
			case 0x4110:
				...
				for( i=0; i<l_qty; ++i )
				{
					//DEBUGLOG( "Vertex %i\n", i );
                    //fflush( stdout );
					file->read( &p_object->vertex[i].x, sizeof(float) );
					//DEBUGLOG( "Vertices list x: %f\n",p_object->vertex[i].x );
					file->read( &p_object->vertex[i].y, sizeof(float) );
					//DEBUGLOG( "Vertices list y: %f\n",p_object->vertex[i].y );
					file->read( &p_object->vertex[i].z, sizeof(float) );
			...
			{
				file->read( &l_qty, sizeof (unsigned short) );
				ASSERT( l_qty < MAX_POLYGONS );
				p_object->polygons_qty = l_qty;
				DEBUGLOG("Number of polygons: %d\n",l_qty);
				for (i=0; i<l_qty; i++)
				{
					file->read( &p_object->polygon[i].a, sizeof (unsigned short)  );
					//DEBUGLOG( "Polygon point a: %d\n",p_object->polygon[i].a );
					ASSERT( p_object->polygon[i].a >= 0 && p_object->polygon[i].a < p_object->vertices_qty );

					file->read( &p_object->polygon[i].b, sizeof (unsigned short)  );
					//DEBUGLOG("Polygon point b: %d\n",p_object->polygon[i].b);
					ASSERT( p_object->polygon[i].b >= 0 && p_object->polygon[i].b < p_object->vertices_qty );

					file->read( &p_object->polygon[i].c, sizeof (unsigned short)  );
					//DEBUGLOG("Polygon point c: %d\n",p_object->polygon[i].c);
					ASSERT( p_object->polygon[i].c >= 0 && p_object->polygon[i].c < p_object->vertices_qty );
			...
			case 0x4140:
				file->read( &l_qty, sizeof (unsigned short) );
				ASSERT( l_qty < MAX_VERTICES );
				ASSERT( l_qty == p_object->vertices_qty );
				for (i=0; i<l_qty; i++)
				{
