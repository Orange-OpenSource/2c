...
{	
    ...
    GLDisableDepth();
	{
		...
		{
			const PathNode *toNode = currentNode->connections[path.directions[i]].node;
...
void CCPathFinderNetwork::addCollideable(CCSceneCollideable *collideable, const CCVector3 &extents)
{
...
void CCPathFinderNetwork::addFillerNodes(CCSceneCollideable *collideable)
{
...
void CCPathFinderNetwork::connect(CCSceneCollideable *objectToPath)
{
			...
			if( currentNode != targetNode )
			{
				...
				if( distance < maxNodeDistance )
				{
					...
					if( currentNode->numberOfConnections < PathNode::max_connections || connectionIndex < currentNode->numberOfConnections )
					{
						...
						CCSceneCollideable *collidedWith = CCMovementOctreeCollisionCheck( objectToPath, currentNode->point, targetNode->point );
...
const uint CCPathFinderNetwork::findClosestNodes(const CCVector3 &position, const float &radius, const CCVector3 **vectors, const uint &length)
{
	uint found = 0;
...
const CCPathFinderNetwork::PathNode* CCPathFinderNetwork::findClosestNode(const CCVector3 *position, const bool withConnections)
{
	int closestNode = -1;
...
const bool CCPathFinderNetwork::followPath(Path &path, 
