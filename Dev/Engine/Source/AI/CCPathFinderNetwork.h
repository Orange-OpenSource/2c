...
public:
	...
	void addCollideable(CCSceneCollideable *collideable, const CCVector3 &extents);
	void addFillerNodes(CCSceneCollideable *collideable);
	
	void clear();
	
	// Connect our nodes
	void connect(CCSceneCollideable *objectToPath);
	
	const uint findClosestNodes(const CCVector3 &position, const float &radius, const CCVector3 **vectors, const uint &length);
	...
	const PathNode* findClosestNode(const CCVector3 *position, const bool withConnections);
...
protected:
	...
	const bool followPath(Path &path, const int currentDirection, 
