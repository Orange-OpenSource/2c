...
struct CCVector3
{
	...
	const bool operator!=(const CCVector3& other) const
	{
	...
	const bool toTarget(const float target, const float speed);
	const bool toTarget(const CCVector3 &target, const float speed);
	const bool toTarget(const CCVector3 &target, const float speedX, const float speedY, const float speedZ);
...
static inline const float CCVector3LengthSquared(const CCVector3 &vector)
{
	return ( vector.x * vector.x ) + ( vector.y * vector.y ) + ( vector.z * vector.z );
}

static inline const float CCVector3Magnitude(const CCVector3 &vector, const bool squared=true)
{
	if( squared )
	{
...
struct CCColour
{
	...
	CCColour(const float r, const float g, const float b, const float a)
	{
		red = r;
		green = g;
		blue = b;
    ...
    inline const bool equals(const CCColour &other) const
	{
    ...
    const bool operator!=(const CCColour &other) const
	{
		return red != other.red || green != other.green || blue != other.blue || alpha != other.alpha;
	}
	
	const bool toTarget(const CCColour &target, const float amount);
};
...
{
	inline const bool notEqual() const
	{
		return current != target;
	}
	
	inline const bool isTargetZero() const
	{
	...
	{
		return CCVector3DistanceCheck2D( current, target, false );
	}
